...
dist_root_path='F:\电机检测项目\dataset\motor_detection_dataset_v1_no_chinese'
count=0
...
def copy_dataset(raw_root_path_element,dist_root_path,image_list):
    ...
    for element in image_list:
        ...
        if element.split('.')[-1] == 'tif':
            ...
            element_dist=str(count)+'.tif'

            dist_element_image_path = os.path.join(dist_root_path, 'image',element_dist)
            annotation_path_list = os.listdir(annotation_path)
            element_xml=element.rstrip('tif') + 'xml'
            element_xml_dist = element_dist.rstrip('tif') + 'xml'
            element_annotation_path = os.path.join(annotation_path,element_xml)
            dist_element_annotation_path = os.path.join(dist_root_path, 'annotation', element_xml_dist)
